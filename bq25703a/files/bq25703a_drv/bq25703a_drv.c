...
int set_charge_voltage_current(int voltage, int current_mA)
{
	...
	switch (voltage)
		{
		case CHARGE_VOL_1:
			little_to_big(0x2B00, val);// 11008 mv
			break;

		case CHARGE_VOL_2:
			little_to_big(0x2EE0, val);// 12000 mv
			break;
		
		case CHARGE_VOL_3:
			little_to_big(0x32D0, val);// 13008 mv
			break;

		default:
			little_to_big(0x2EE0, val);// 11008 mv
			break;
		}
...
int main(void)
{
    ...
    i2c_open();
	printf("charge function:	1\notg function:	2\nplease select which function:");
	scanf("%d",&which);
	printf("please input the voltage(11 12 13 (V) ):");
	scanf("%d",&voltage);
	printf("please input the current_mA(1 2 3 (A) ):");
	scanf("%d",&current_mA);
	...
	switch (which)
	{
		...
		case 2:
			printf("Are you remove input DC (1 0):");
			scanf("%d",&is_dc);
			if (is_dc == 1)
			{
					bq25703a_otg_function_init(voltage, current_mA);
			}else{
				printf("remove input DC !!\n");
			}
			break;
	}
	//start irq thread
